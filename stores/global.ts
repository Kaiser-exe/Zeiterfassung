...
export const useGlobalStore = defineStore(
    ...
    () => {
        ...
        const fetchUser = async (userId: string) => {
            ...
            console.log("test")
            let user = await (
                await useFetch("/api/getUser", {params: {param1: userId}})
            ).data.value
            const worked = await (
                await useFetch("/api/getWorked", {params: {param1: userId}})
            ).data.value
            const userExpected = await (
                await useFetch("/api/getUserExpected", {params: {param1: userId}})
            ).data.value
            ...
            if (user) {
                ...
                expectedWeekdays = expectedWeekdays.filter(
                    (expectedWeekday) =>
                        userExpected &&
                        userExpected.some(
                            (userExpect) => userExpect.ex_id === expectedWeekday.id
                        )
                )
        ...
        const fetchAdminOverview = async () => {
            ...
            const userExpected = await (
                await useFetch("/api/getUserExpected")
            ).data.value
            const expected = await (await useFetch("/api/getExpected")).data.value
            const weekdaysTemp = await (await useFetch("/api/getWeekday")).data.value
            ...
            for (const user of users) {
                ...
                if (userExpected) {
                    for (const expect of expected.filter(
                        (expects) =>
                            userExpected &&
                            userExpected.some(
                                (userExpect) =>
                                    expects.ex_id === userExpect.ex_id &&
                                    userExpect.us_id === user.us_id
                            )
                    )) {
                        expectedWeekdays.push({
                            id: expect.ex_id,
        ...
        const getSessionStore = () => {
            return sessionStorage.getItem("global");
            console.log(sessionStorage.getItem("global"))
        }
